...
import numpy as np
#sys.path.append("..")
from .utils import drug_feature_extract, get_MACCS, gene_map, read_bed_file, read_csv_header, get_sim_mat
from rdkit import Chem
from torch_geometric import data as DATA
from torch_geometric.data import Batch


class CustomData(DATA.Data):
    """Keep line-graph indices valid after Batch collation."""

    def __inc__(self, key, value, *args, **kwargs):
        if key == 'line_graph_edge_index':
            return self.edge_index.size(1) if self.edge_index.nelement() != 0 else 0
        if key == 'edge_index_batch':
            return 1
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, DATASET_NAME, args):
        ...
        random.seed(10)
        # GraphSynergy 风格邻域（见 model.nets.graph_synergy_resource）
        self.gs_resource = None
        if getattr(args, 'use_dual_drug_gcn', False):
            from model.nets.graph_synergy_resource import GraphSynergyResource
            gpath = args.data_dir + DATASET_NAME + '/'
            gene_df = pd.read_csv(gpath + 'cell line_gene_expression.csv', index_col=0)
            cell_names = list(gene_df.index.drop_duplicates())
            self.gs_resource = GraphSynergyResource(
                gpath, len(self.drug_fea), cell_names, n_hop=2, n_memory=32, seed=args.seed
            )
    ...
    def build_data(self, args):
        ...
        for index, itr in enumerate(synergy_temp):
            ...
            if getattr(args, 'use_dual_drug_gcn', False) or getattr(args, 'use_hetero_graph_encoder', False):
                itr.append((i, j, k))
            itr.append(index)
        train_size = 0.8
        ...
        if getattr(args, 'use_dual_drug_gcn', False) or getattr(args, 'use_hetero_graph_encoder', False):
            ...
            test_set, test_index = test_set[:, :6], test_set[:, 6]
            dataset_all = dataset_all[:, :6]
        else:
            train_set = train_set[:, :5]
            test_set, test_index = test_set[:, :5], test_set[:, 5]
            dataset_all = dataset_all[:, :5]

        return train_set, test_set, test_index, dataset_all
    ...
    def get_inter_set(self):
        ...
        for index, itr in enumerate(inter_synergy):
            ...
            itr[2] = cline_fea[k]
            itr.insert(3, cline_mask[k])
            if getattr(self.args, 'use_dual_drug_gcn', False) or getattr(
                self.args, 'use_hetero_graph_encoder', False
            ):
                itr.append((i, j, k))
    ...
    def process_drug(self, datas):
        data_list = []
        for data in datas:
            features = torch.tensor(data[0], dtype=torch.float32)   # [num_nodes, feat_dim]
            edge_index = torch.tensor(data[1], dtype=torch.long)    # [2, num_edges]

            # DGNN-DDI style line graph: (u->v) can pass message to (v->w), excluding reverse edge.
            line_graph_edge_index = torch.empty((2, 0), dtype=torch.long)
            edge_attr = torch.empty((0, features.size(1)), dtype=torch.float32)
            if edge_index.nelement() != 0:
                src, dst = edge_index[0], edge_index[1]
                edge_attr = (features[src] + features[dst]) / 2.0
                conn = (dst.unsqueeze(1) == src.unsqueeze(0)) & (src.unsqueeze(1) != dst.unsqueeze(0))
                line_graph_edge_index = conn.nonzero(as_tuple=False).T.contiguous()

            # edge-level batch ids are required by model.DMPNN
    ...
    def collate(self, samples):
        ncols = len(samples[0])
        use_stubs = getattr(self.args, 'use_dual_drug_gcn', False) or getattr(
            self.args, 'use_hetero_graph_encoder', False
        )
        if ncols == 6 and use_stubs:
            drug_a, drug_b, clines, cline_masks, labels, stubs = map(list, zip(*samples))
        else:
            drug_a, drug_b, clines, cline_masks, labels = map(list, zip(*samples))
            stubs = None
        labels = torch.tensor(np.array(labels), dtype=torch.float32)
        drug_a = self.process_drug(drug_a)
        drug_b = self.process_drug(drug_b)
        ...
        if stubs is not None and self.gs_resource is not None:
            drug_a.gs_context = self.gs_resource.batch_context(stubs, torch.device('cpu'))

        if stubs is not None and getattr(self.args, 'use_hetero_graph_encoder', False):
            ia = [t[0] for t in stubs]
            ib = [t[1] for t in stubs]
            ic = [t[2] for t in stubs]
            drug_a.raw_drug_idx = torch.tensor(ia, dtype=torch.long)
            drug_b.raw_drug_idx = torch.tensor(ib, dtype=torch.long)
            clines.cell_line_idx = torch.tensor(ic, dtype=torch.long)
