...
def result_save_i(args, epoch, out_file, train_result, test_result):
    ...
    # 回归
        ...
        train_result_data = pd.DataFrame([train_result])
        test_result_data = pd.DataFrame([test_result])
        train_result_data.to_csv(out_file + '/i_train_r', mode='a', header=False, index=False)
        test_result_data.to_csv(out_file + '/i_test_r', mode='a', header=False, index=False)


def result_save_independent(args, out_file, train_result, val_result, suffix=""):
    """独立划分模式：train / val 逐 epoch；suffix 例 ''.csv' 与 train1 输出文件名一致。"""
    if args.mode == 0:
        pd.DataFrame([train_result]).to_csv(out_file + '/i_train' + suffix, mode='a', header=False, index=False)
        pd.DataFrame([val_result]).to_csv(out_file + '/i_val' + suffix, mode='a', header=False, index=False)
    else:
        pd.DataFrame([train_result]).to_csv(out_file + '/i_train_r' + suffix, mode='a', header=False, index=False)
        pd.DataFrame([val_result]).to_csv(out_file + '/i_val_r' + suffix, mode='a', header=False, index=False)
